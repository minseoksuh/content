...
The **`reduce()`** method executes a “reducer” callback function (that you provide) on each element of the array, resulting in a single output value (the result of running the reducer to completion over the entire array).
...
- *initialValue* {{optional_inline}}
  - : A value to which *previousValue* is initialized the first time the callback is called. If specified, *currentValue* is initialized to the first value in the array. If *not* specified, *previousValue* is initialized to the first value in array, and *currentValue* is initialized to the second value in the array.
